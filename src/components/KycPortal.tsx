...
const LIVENESS_STEPS = [
  ...
  { id: 'blink', label: 'BLINK both eyes to confirm human liveness', duration: 2500 }
];
...
export default function KycPortal({
  ...
  const [docType, setDocType] = useState<KYCDetails['documentType']>('national_id');
  ...
  useEffect(() => {
    if (kycDetails.status === 'verifying') {
      const pipelineSteps = [
        { msg: 'OCR Extraction: Parsing doc fields...', duration: 2000 },
        { msg: 'Facial Topology Audit: Comparing liveness mesh to passport photo...', duration: 2500 },
        { msg: 'Risk Score Assessment: Checking global sanction address lists...', duration: 2005 },
  ...
  return (
    <div className="flex flex-col h-full bg-transparent font-sans text-white overflow-y-auto no-scrollbar pb-6 p-6">
      <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-5 sticky top-0 bg-[#0a0802]/40 backdrop-blur-md z-10">
        <div>
          <h2 className="text-base font-display font-bold text-white uppercase tracking-widest">Identity Ledger (KYC)</h2>
      ...
      {kycDetails.status === 'none' && !selfieCaptured && (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 flex-1 justify-center py-4 relative z-10">
          <div className="bg-white/5 border border-white/10 p-5 rounded-2xl flex flex-col gap-4 backdrop-blur-sm shadow-xl">
            <h3 className="text-[10px] font-bold text-amber-400 uppercase tracking-widest flex items-center gap-2 mb-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-black/60 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-xs text-white placeholder-white/20 focus:outline-none focus:border-amber-400"
                />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative font-sans">
                ...
                <select
                  ...
                  className="w-full bg-black/60 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-xs text-white/85 focus:outline-none focus:border-amber-400 appearance-none cursor-pointer"
                >
                  <option value="India">India (₹ INR Home)</option>
                ...
                {(['passport', 'drivers_license', 'national_id'] as const).map((t) => (
                  <button
                    ...
                    className={`py-2 px-3 border rounded-xl text-[9px] font-bold text-center transition-all uppercase tracking-wider ${
                      docType === t
                        ? 'bg-amber-500/15 border-amber-500/50 text-[#facc15]'
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                type="text"
                required
                placeholder="ID String (Passport ID / Aadhaar ID)"
                value={docNumber}
                onChange={(e) => setDocNumber(e.target.value)}
                className="w-full bg-black/60 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white placeholder-white/20 focus:outline-none focus:border-amber-400 font-mono"
              />
            </div>
          </div>
          ...
          <button
            ...
            type="submit"
            className="w-full bg-gradient-to-tr from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-black font-bold py-3 rounded-xl text-[10px] uppercase tracking-widest cursor-pointer transition-all"
          >
      ...
      {kycDetails.status === 'none' && !selfieCaptured && cameraActive && (
        ...
        <div className="fixed inset-0 bg-[#0a0802]/95 z-[55] flex flex-col justify-center p-6 items-center backdrop-blur-md">
          <div className="w-full max-w-sm bg-[#0a0802]/90 border border-white/10 p-6 rounded-2xl flex flex-col items-center gap-5 relative shadow-[0_0_55px_rgba(245,158,11,0.15)]">
            
            <div className="text-center w-full">
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-amber-500">Liveness Biometric Calibration</h3>
              <p className="text-[9px] text-amber-400 font-mono mt-1 uppercase tracking-wider font-semibold animate-pulse">Scanning Grid: {Math.floor(scanProgress)}%</p>
            </div>

            {/* Scanner Feed stage */}
            <div className="w-60 h-72 rounded-[30px] bg-[#030302] border border-white/10 overflow-hidden relative flex items-center justify-center">
              {cameraError ? (
                /* Falling Back: Simulated Yellow Neon Face Grid */
                <div id="simulated-camera-feed" className="w-full h-full relative overflow-hidden flex flex-col items-center justify-center">
                  <div className="absolute inset-0 bg-[radial-gradient(#eab308_1px,transparent_1px)] [background-size:16px_16px] opacity-10"></div>
                  
                  {/* Sweep scan bar */}
                  <motion.div
                    animate={{ y: [-100, 300] }}
                    transition={{ repeat: Infinity, duration: 2.2, ease: "linear" }}
                    className="absolute w-full h-[2px] bg-gradient-to-r from-transparent via-amber-400 to-transparent shadow-[0_0_15px_rgba(245,158,11,0.8)]"
                  />

                  {/* Wireframe face mockup */}
                  <div className="w-36 h-48 rounded-full border border-amber-500/30 flex flex-col items-center justify-center relative bg-amber-950/5">
                    {/* Glowing dots */}
                    <span className="w-2 h-2 rounded-full bg-amber-400 absolute top-10 left-8 animate-ping"></span>
                    <span className="w-2 h-2 rounded-full bg-amber-400 absolute top-10 right-8 animate-ping"></span>
                    <span className="w-2 h-2 rounded-full bg-amber-500 absolute top-24 left-16"></span>
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-400 absolute bottom-10 left-12"></span>
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-400 absolute bottom-10 right-12"></span>
                    
                    <span className="text-[8px] font-mono text-amber-400/60 mt-16 text-center tracking-wider leading-relaxed">
                      [ FACE ANALYSIS ]<br />LIVENESS DETECTED
                    </span>
                  </div>
                </div>
              ) : (
                ...
                <div className="w-full h-full relative">
                  ...
                  <div className="absolute inset-0 border-[24px] border-[#0a0802]/80" style={{ pointerEvents: 'none' }}>
                    <div className="w-full h-full border border-dashed border-amber-500/50 rounded-full" />
                  </div>
                  <div className="absolute top-2 left-2 bg-black/70 text-amber-400 px-1.5 py-0.5 rounded text-[8px] font-mono border border-amber-500/20">
                    🔴 SENSOR CALIBRATING
                  </div>
                </div>
              )}
            ...
            <div className="w-full bg-white/5 p-4 rounded-xl border border-white/10 text-center font-sans text-xs font-semibold py-3 flex flex-col justify-center min-h-[70px]">
              <span className="text-[8px] font-mono tracking-widest text-white/40 block uppercase mb-1">Human Validation Command</span>
              <AnimatePresence mode="wait">
                <motion.p
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="text-amber-300 font-bold uppercase tracking-wider text-[10px]"
                >
                ...
                {LIVENESS_STEPS.map((_, idx) => (
                  <span 
                    ...
                    className={`h-1.5 rounded-full transition-all ${
                      idx === livenessIndex ? 'w-4 bg-amber-400' : idx < livenessIndex ? 'w-1.5 bg-amber-800' : 'w-1.5 bg-white/10'
                    }`}
                  />
                ))}
              </div>
            ...
            <button
              ...
              onClick={stopCamera}
              className="w-full py-2 rounded-xl bg-white/5 text-white/50 hover:bg-white/10 border border-white/10 text-[9px] uppercase tracking-widest font-bold cursor-pointer font-sans"
            >
              Cancel
            </button>
          </div>
      ...
      {kycDetails.status === 'none' && !cameraActive && !selfieCaptured && (
        ...
        <div className="mt-4 bg-white/5 p-5 rounded-2xl border border-white/10 text-center flex flex-col items-center gap-4 relative z-10 backdrop-blur-sm">
          <div className="w-10 h-10 bg-amber-950/20 text-amber-400 border border-amber-900 rounded-xl flex items-center justify-center animate-bounce">
            <Camera className="w-5 h-5" />
          </div>
          <div>
            <h4 className="text-[10px] font-bold text-amber-400 uppercase tracking-widest">Webcam Liveness Check Instructions</h4>
            <p className="text-[11px] text-white/45 mt-1 leading-relaxed font-sans px-2">
              Our network registers consensus validity using a facial vector topology match to reject bot script clusters and maintain ledger token value representation. This takes 10 seconds.
            </p>
          </div>
          <button
            ...
            type="button"
            onClick={startCamera}
            className="w-full bg-[#0a0802] hover:bg-white/5 border border-amber-500/30 text-amber-300 font-bold py-2.5 rounded-xl text-[10px] uppercase tracking-widest flex items-center justify-center gap-1 cursor-pointer transition-all font-sans"
          >
            <Camera className="w-4 h-4 text-amber-400" /> Start Calibration Scan
          </button>
        </div>
      )}
      ...
      {kycDetails.status === 'verifying' && (
        <div className="flex-1 flex flex-col justify-center py-4 relative z-10">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col gap-6 items-center text-center">
            ...
            <div>
              <h3 className="text-sm font-bold uppercase tracking-widest text-[#facc15]">Parsing Identity Verification Vectors</h3>
            ...
            <div className="w-full bg-[#030302]/80 border border-amber-500/10 p-4 rounded-xl font-mono text-left select-none text-[9.5px] text-amber-300 h-32 flex flex-col justify-between">
              <div>
                ...
                <div className="mt-2 text-white/80 flex items-center gap-1.5 font-mono">
                  ...
                  <span className="text-amber-400 font-mono tracking-wide">{pipelineLog}</span>
                </div>
              </div>
              <div className="border-t border-white/5 pt-2 flex justify-between text-white/35 text-[8.5px] font-mono lowercase">
                ...
                <span>Active Nodes: 512 Verify OK</span>
              </div>
            </div>
      ...
      {kycDetails.status === 'approved' && (
        <div className="flex-1 flex flex-col justify-center py-4 relative z-10">
          <div className="bg-white/5 border border-amber-500/20 rounded-2xl p-6 flex flex-col gap-5 items-center text-center backdrop-blur-sm">
            
            <div className="w-12 h-12 bg-amber-950/40 border border-amber-400 rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(245,158,11,0.2)]">
              <CheckCircle className="w-6 h-6 text-amber-400" />
            </div>

            <div>
              <h3 className="text-sm font-bold uppercase tracking-widest text-[#facc15]">Identity Node Validated</h3>
              <p className="text-[11px] text-white/50 mt-1 px-4 leading-relaxed font-sans">
                Network validation complete. Facial topology audits match corresponding issuer details on the minipocicoin.in protocol.
              </p>
            </div>

            <div className="w-full bg-[#0a0802] p-4 rounded-xl border border-white/10 text-left text-[9.5px] gap-2 flex flex-col font-mono text-white/50">
              <div className="flex justify-between border-b border-white/5 pb-1.5">
                <span className="text-[8.5px] text-white/30 font-mono uppercase font-bold">Biometric Mesh Match</span>
                <span className="text-amber-400 font-bold font-mono">100% SECURE_VALIDATOR_OK</span>
              </div>
              <div className="flex justify-between border-b border-white/5 pb-1.5">
              ...
              <div className="flex justify-between font-sans">
                ...
                <span className="text-amber-400 font-bold font-mono uppercase font-sans">UNLOCKED FOR INR TRANSFER</span>
              </div>
            </div>

            <p className="text-[11px] text-white/50 leading-relaxed bg-[#0a0802] p-3 rounded-xl border border-amber-500/10 font-sans">
              Your self-mined balances are unlocked. Access your <strong>Simulated Node Wallet</strong> to migrate these coins directly to your spendable Indian Rupees (INR) balance.
            </p>
          </div>
        </div>
      )}
